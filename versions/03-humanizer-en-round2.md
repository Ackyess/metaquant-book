...
- **🔬 A Lesson from Practice** (in some chapters): Turns mistakes the lab encountered into cases that show how an error later became a rule.
...
- The constituent lists of some indexes are adjusted retrospectively.

When handling index constituents, separate the announcement date, effective date, constituent list available at the time, and later revisions. A list downloaded today cannot stand in for the investable universe people could see then. If a database stores only final values, a backtest may use revisions that had not yet been published at the time. A rigorous data system must also preserve the version visible at each point in time.
...
> **For the same Sharpe ratio, the evidence is weaker if it was selected from more attempts.**
...
The losing periods of the two strategies may occur at different times, but this cannot be judged from a story alone. It must be tested using return series from the same period and under the same cost assumptions. Only when the low-correlation relationship survives testing can the combination reduce the volatility contributed by either strategy alone.

Low correlation cannot rescue a negative-expectancy strategy. You still need to check net returns after costs, tail risk, liquidity, and whether correlations jump under stress.
