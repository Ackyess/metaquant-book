...
When handling index constituents, separate the announcement date, effective date, constituent list available at the time, and later revisions. A list downloaded today cannot stand in for the investable universe people could see then. If your database stores only “what these numbers look like now,” rather than “what they looked like at that moment in the past,” then your backtest will unknowingly use versions that were only corrected later—another form of look-ahead bias. A truly rigorous database does not store the “final version of the truth,” but rather what we knew at every point in time. It is troublesome, space-consuming, and unglamorous, but it is necessary work if you want to draw the map correctly.
